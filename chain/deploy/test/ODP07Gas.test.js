...
describe('0.7 measured registration costs',()=>{
 it('records gas for the new indexed concern model rather than reusing the legacy number',async()=>{
  ...
  // Comparable pair after the first mint warmed the profile slots: without and with the public preview copy.
  const mintedPlain=await tx(c.mintPhysical(await inputs(),ethers.hexlify(ethers.randomBytes(32))));
  const mintedWithPreview=await tx(c.mintPhysical(await inputs('physical',{previewHash:hash('preview')}),ethers.hexlify(ethers.randomBytes(32))));
  ...
  console.log('V07_GAS '+JSON.stringify({registration:String(registration.gasUsed),physicalMint:String(minted.gasUsed),nextPhysicalMint:String(mintedPlain.gasUsed),nextPhysicalMintWithPreview:String(mintedWithPreview.gasUsed),firstConcern:String(first.gasUsed),secondRaiser:String(second.gasUsed),withdraw:String(withdrawn.gasUsed),reRaise:String(repeated.gasUsed)}));
 });
});
