...
The byte counts and build evidence in the [sealed audit checks](../review/audit-handoff-abi6/TESTS_AND_ANALYSIS.md) are those of ABI `0.7-redesign-6`. `0.7-redesign-7` and the current `0.7-redesign-8` (which adds `previewHash`) add code to the core; their sizes are not remeasured on this page. Earlier `review/v07/` measurements describe their own snapshot. The release roster is ten contracts (core plus nine satellites).
