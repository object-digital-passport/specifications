...
root=Path(__file__).resolve().parents[2];out=root/'review/v07-stage4/slither';out.mkdir(exist_ok=True)
venv,solc=map(Path,sys.argv[1:3]);env=dict(os.environ,VIRTUAL_ENV=str(venv))
names=['ObjectDigitalPassport','ODPEditionUnits','ODPAuthorAttestation','ODPPassportConcerns','ODPHosting','ODPProfileDirectory','ODPRegistryRelations','ODPPassportProofRegistry','ODPWalletDocumentAnchor','ODPStatementJournal']
def run(name):
